#setwd("~/Documents/SURP/data_exploration")
...
file_input = args[1] #"EAST_ASIA_LAMA1.frq.strat" #args[1] # ex europe_TYK2.frq.strat" or "europe_rs34536443.frq.strat"
...
region_name = unlist(strsplit(region_gene_name, "_"))[1]
gene_name = unlist(strsplit(region_gene_name, "_"))[2]

freqFile <-read.delim(file_input, sep = "", header = T)


# #the number of SNP
# numSNPs <- length(unique(freqFile$SNP))
# SNPs <- unique(freqFile$SNP)
...
for (i in 1:length(list_by_snp) ){
    ...
    for (k in 1:(nrow(df_new)) ){
      lower_uncert <- c(lower_uncert, binom.approx(df_new$adjusted_mac[k], df_new$new_nchrobs[k], conf.level = 0.95)[,4])
      upper_uncert <- c(upper_uncert, binom.approx(df_new$adjusted_mac[k], df_new$new_nchrobs[k], conf.level = 0.95)[,5])
    }
...
###OLD VERSION (PRIOR TO CREATING SLIDING WINDOWS)
# args = commandArgs(trailingOnly = TRUE)
# library(ggplot2)
# 
# file_input = args[1] #ex EastAsianHistoryAllelesrs12030098.frq.strat
# output_name = args[2]
# 
# freqFile <-read.delim(file_input, sep = "", header = T)
# 
# #put the axis in order
# freqFile$CLST <- factor(freqFile$CLST, levels = c("Modern","(0,1e+03]", "(1e+03,2e+03]", 
#                                                   "(2e+03,3e+03]", "(3e+03,4e+03]", 
#                                                   "(4e+03,5e+03]","(5e+03,6e+03]", 
#                                                   "(6e+03,7e+03]", "(7e+03,8e+03]",
#                                                   "(8e+03,9e+03]", "(9e+03,1e+04]"))
# 
# my_plot <- ggplot(freqFile, aes(x = CLST, y = MAF , group = CHR) ) +
#   geom_point() +
#   geom_line() +
#   scale_x_discrete(limits = rev(levels(freqFile$CLST)) )
# 
# pdf(output_name)
# print(my_plot)
# dev.off()